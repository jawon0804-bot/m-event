...
- 표 컬럼: 구분(daily/weekly/monthly) · 설비명 · **설비ID**(그 점검표의 `fids` 개수) · **매핑**(엑셀 생성 건수) · **점검**(점검한 횟수) · **최근 점검**(마지막 일시+점검자) · 상태
...
- **상태는 두 축으로 갈라요** — 이게 이 화면에서 제일 헷갈리던 부분이에요.
  - **점검표 행 = 매핑 축**(엑셀이 만들어졌나): `정상` / `진행중` / `미매핑` / `⚠️ 부족`
  - **설비 행 = 점검 축**(현장에서 점검했나): `완료` / `⚠️ 미완료` / `대기`
