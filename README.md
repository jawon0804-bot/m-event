...
UserDB에서 이름이 일치하는 사람을 최대 5명까지 조회
        ↓
그 중 전화번호가 "완전히 일치"하거나 "뒤 4자리만 일치"해도 통과
```

> ⚠️ **주의할 점**: 전화번호 검증이 "끝 4자리만 같아도 로그인 허용"하도록 되어 있어요. 같은 이름을 가진 직원이 여러 명 있을 경우, 의도와 다르게 다른 사람 계정으로 잘못 들어갈 가능성이 있어요. 보안을 더 엄격하게 하고 싶다면 이 부분을 점검할 필요가 있어요.
...
- 자동화(이슈 생성, 메일 발송)는 전부 Firebase Functions(`index.js`)가 처리해요.
- 설비ID를 사람이 읽기 좋은 이름으로 바꿔서 보여줄 때는, **Dashboard(facility-dashboard) 서비스의 API를 그대로 빌려 써요.**
...
> 🧸 비유: m-event는 "내 사전(설비 이름 목록)"을 따로 안 만들고, 옆집(Dashboard)의 사전을 그때그때 빌려서 찾아보는 거예요. 옆집 사전이 바뀌면 m-event가 보여주는 이름도 같이 바뀌어요.
...
- [ ] GitHub 레포 주소
- [ ] 로그인 시 전화번호 "뒤 4자리 일치"만으로 통과되는 로직이 의도된 설계인지, 보안 허점인지
