...
// ⚠️ exceljs는 여기서 최상위 require를 하지 않는다 — 실제로 쓰는 함수 안에서 받는다.
...
const { cloneSheet } = require("./excel-utils");
...
//
// [2026-07-27] 실제 복제 로직은 lib/excel-utils.js의 cloneSheet로 옮겼다. 엑셀 탭
// "전체 다운로드"(excel-merge.js)가 같은 복제가 필요해지면서 공유 모듈로 뺀 것.
// 여기서는 옵션 없이 호출하므로 복사 범위는 예전과 완전히 동일하다
// (이미지/조건부서식 복사는 excel-merge.js 전용 옵션이라 근무일지에는 적용되지 않음).
function cloneTemplateSheet(targetWorkbook, tplSheet, sheetName) {
  return cloneSheet(targetWorkbook, tplSheet, sheetName, { warnPrefix: "[근무일지 내보내기]" });
}
