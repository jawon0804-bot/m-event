...
// ──────────────────────────────────────────────
function fillWorkLogSheet(ws, data, center) {
  // 0) 제목(B2) — 센터명 + 고정 문구 조합으로 동적 생성 (하드코딩 금지)
  ws.getCell(2, 2).value = `${center} ${WL_TITLE_SUFFIX}`;
  ...
  async () => {
    ...
    for (const center of centers) {
      try {
        ...
        fillWorkLogSheet(ws, data, center);
